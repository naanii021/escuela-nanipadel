...
router.post("/registro", async (req, res) => {
  try {
    ...
    const [result] = await query(
      ...
      [nombre, email, telefono || null, password_hash]
    );

    // Generar token directamente al registrarse
    const token = jwt.sign(
      { id: result.insertId, nombre, email, rol: "usuario" },
      JWT_SECRET,
      { expiresIn: "7d" }
    );
    ...
    res.status(201).json({
      ...
      token,
      user: { id: result.insertId, nombre, email, rol: "usuario" },
    });
  } catch (e) {
...
router.get("/me", async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      user: { id: decoded.id, nombre: decoded.nombre, email: decoded.email, rol: decoded.rol },
    });
  } catch (e) {
